...
function Formua(params) {
  if (typeof window !== "undefined") {
    ...
    const updateInputs = () => {
      let inputElements = Array.from(form?.querySelectorAll("input,textarea,select") || []);
      inputElements = inputElements.filter(e => {
        if (e instanceof HTMLInputElement) {
          const disallowedInputTypes = ["file", "submit", "reset", "button", "image"];
          return !disallowedInputTypes.includes(e.type);
        }
        return true;
      });
    ...
    const updateFormData = () => {
      ...
      inputs?.forEach(input => {
        if (input instanceof HTMLInputElement && input.type === "radio") {
          ...
          newPureData[input.name] = getValue(
            inputs.find(i => i instanceof HTMLInputElement && i.type == "radio" && i.name == input.name && i.checked),
            false
          );
        } else {
    ...
    const updateSingleData = e => {
      ...
      setFormData(prevData => ({
        ...prevData,
        [inputElement.name]: getValue(inputElement)
      }));
      setPureData(prevData => ({
        ...prevData,
        [inputElement.name]: getValue(inputElement, false)
      }));
    };
    ...
    const addEventListeners = () => {
      inputs?.forEach(input => {
        ...
        if (eventHandlers.length > 0) {
          ...
          setBlurInputEventHandlersMap(previous => ({
            ...previous,
            [input.name]: eventHandler
          }));
    ...
    const checkFormErrors = () => {
      const independentsResult = independentValidators
        ?.map(validator => validator.callback())
        .reduce(
          (acc, next) => {
      ...
      const dependenciesResult = dependentValidators
        ?.map(validator => validator.callback())
        .reduce(
          (acc, next) => {
            acc.isValid = acc.isValid && next.result;
      ...
      return {
        isValid: dependenciesResult.isValid && independentsResult.isValid,
        errors: { ...dependenciesResult.errors, ...independentsResult.errors }
      };
    };
